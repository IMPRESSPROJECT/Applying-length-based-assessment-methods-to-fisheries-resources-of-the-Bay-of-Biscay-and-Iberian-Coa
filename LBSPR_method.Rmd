---
title: "LBSPR method: Assessing the sensitivity of length-indicator methods for resources in the Atlantic waters"
author: ""
date: "`r Sys.Date()`"
output:
  html_document:
    number_sections: yes
    toc: yes
    toc_float: yes
  pdf_document:
    number_sections: yes
    toc: yes
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


```{r,include=FALSE}
library(LBSPR) 
```



# Nephrops norvegicus FU25 Females

Life history parameters:
  
$L_{inf}^{LIT}$ = 71 mm
${M/k}^{LIT}$ = 1.9 
$L_{50}$=28 mm
$L_{95}$=32.2 mm


```{r message=FALSE, warning=FALSE}
Hke1Pars <- new("LB_pars")
Hke1Pars@Linf <- 71
Hke1Pars@L50 <- 28
Hke1Pars@L95 <- 32.2
Hke1Pars@MK <- 1.9 
Hke1Pars@L_units <- "mm"
```

Reference model:

```{r message=FALSE, warning=FALSE}
HkeLenFreq1 <- new("LB_lengths", LB_pars=Hke1Pars, file="FU25_Females.csv", dataType="freq", header=TRUE)
HkeLenFreq1@L_units <- Hke1Pars@L_units
```


```{r message=FALSE, warning=FALSE}
HkeLenFreq1
Fit1 <- LBSPRfit(Hke1Pars, HkeLenFreq1, verbose = FALSE)
```

Underestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Hke1ParsMK <-Hke1Pars
Hke1ParsMK@MK <- 0.75*1.90


HkeLenFreq1MK <- new("LB_lengths", LB_pars=Hke1ParsMK, file="FU25_Females.csv", dataType="freq", header=TRUE)
HkeLenFreq1MK@L_units <- Hke1ParsMK@L_units


Fit1MK <- LBSPRfit(Hke1ParsMK, HkeLenFreq1MK, verbose = FALSE)
```

Overestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Hke2ParsMK <-Hke1Pars
Hke2ParsMK@MK <- 1.25*1.90


HkeLenFreq2MK <- new("LB_lengths", LB_pars=Hke2ParsMK, file="FU25_Females.csv", dataType="freq", header=TRUE)
HkeLenFreq2MK@L_units <- Hke2ParsMK@L_units


Fit2MK <- LBSPRfit(Hke2ParsMK, HkeLenFreq2MK, verbose = FALSE)
```

M/k=1.5.

```{r message=FALSE, warning=FALSE}
Hke3ParsMK <-Hke1Pars
Hke3ParsMK@MK <- 1.5

HkeLenFreq3MK <- new("LB_lengths", LB_pars=Hke3ParsMK, file="FU25_Females.csv", dataType="freq", header=TRUE)
HkeLenFreq3MK@L_units <- Hke3ParsMK@L_units

Fit3MK <- LBSPRfit(Hke3ParsMK, HkeLenFreq3MK, verbose = FALSE)
```

Underestimated $L_{inf}^{LIT}$.


```{r message=FALSE, warning=FALSE}
Hke1ParsLinf <-Hke1Pars
Hke1ParsLinf@Linf <- 0.75*71

HkeLenFreq1Linf <- new("LB_lengths", LB_pars=Hke1ParsLinf, file="FU25_Females.csv", dataType="freq", header=TRUE)
HkeLenFreq1Linf@L_units <- Hke1ParsLinf@L_units


Fit1Linf <- LBSPRfit(Hke1ParsLinf, HkeLenFreq1Linf, verbose = FALSE)
```

Overestimated $L_{inf}^{LIT}$.


```{r message=FALSE, warning=FALSE}
Hke2ParsLinf <-Hke1Pars
Hke2ParsLinf@Linf <- 1.25*71

HkeLenFreq2Linf <- new("LB_lengths", LB_pars=Hke2ParsLinf, file="FU25_Females.csv", dataType="freq", header=TRUE)
HkeLenFreq2Linf@L_units <- Hke2ParsLinf@L_units


HkeLenFreq2Linf@LMids=seq(15.5,89.5,by=1)
A=HkeLenFreq2Linf@LData
for (i in 1:18){
A=rbind(A,HkeLenFreq2Linf@LData[57,])}

A[58:75,1:38]=rep(0,38)


HkeLenFreq2Linf@LData=A
Hke2ParsLinf <-Hke1Pars
Hke2ParsLinf@Linf <- 1.25*71
Fit2Linf <- LBSPRfit(Hke2ParsLinf, HkeLenFreq2Linf, verbose = FALSE)

```

Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)
smoothEsts1MK <- data.frame(Fit1MK@Ests)
smoothEsts2MK <- data.frame(Fit2MK@Ests)
smoothEsts3MK <- data.frame(Fit3MK@Ests)
smoothEsts1Linf <- data.frame(Fit1Linf@Ests)
smoothEsts2Linf <- data.frame(Fit2Linf@Ests)


spr1=Fit1@SPR
sprmk1=Fit1MK@SPR
sprmk2=Fit2MK@SPR
sprmk3=Fit3MK@SPR
sprL1=Fit1Linf@SPR
sprL2=Fit2Linf@SPR

min=min(c(spr1,sprmk1,sprmk2,sprmk3,sprL1,sprL2))
max=max(c(spr1,sprmk1,sprmk2,sprmk3,sprL1,sprL2))

plot(1982:2019,spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(1982:2019, smoothEsts1$SPR, lwd = 1,col="black")
lines(1982:2019, smoothEsts1MK$SPR, lwd = 1,col=2)
lines(1982:2019, smoothEsts2MK$SPR, lwd = 1,col=3)
lines(1982:2019, smoothEsts3MK$SPR, lwd = 1,col=4)
lines(1982:2019, smoothEsts1Linf$SPR, lwd = 1,col=5)
lines(1982:2019, smoothEsts2Linf$SPR, lwd = 1,col=6)


points(1982:2019, sprmk1,col=2,pch=19)
points(1982:2019, sprmk2,col=3,pch=19)
points(1982:2019, sprmk3,col=4,pch=19)
points(1982:2019, sprL1,col=5,pch=19)
points(1982:2019, sprL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```




```{r message=FALSE, warning=FALSE}

fm1=Fit1@FM
fmmk1=Fit1MK@FM
fmmk2=Fit2MK@FM
fmmk3=Fit3MK@FM
fmL1=Fit1Linf@FM
fmL2=Fit2Linf@FM

min=min(c(fm1,fmmk1,fmmk2,fmmk3,fmL1,fmL2))
max=max(c(fm1,fmmk1,fmmk2,fmmk3,fmL1,fmL2))

plot(1982:2019,fm1,ylim=c(min,max),xlab="Years",ylab="F/M estimates",pch=19)

lines(1982:2019, smoothEsts1$FM, lwd = 1,col="black")
lines(1982:2019, smoothEsts1MK$FM, lwd = 1,col=2)
lines(1982:2019, smoothEsts2MK$FM, lwd = 1,col=3)
lines(1982:2019, smoothEsts3MK$FM, lwd = 1,col=4)
lines(1982:2019, smoothEsts1Linf$FM, lwd = 1,col=5)
lines(1982:2019, smoothEsts2Linf$FM, lwd = 1,col=6)


points(1982:2019, fmmk1,col=2,pch=19)
points(1982:2019, fmmk2,col=3,pch=19)
points(1982:2019, fmmk3,col=4,pch=19)
points(1982:2019, fmL1,col=5,pch=19)
points(1982:2019, fmL2,col=6,pch=19)

legend("topleft", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```



```{r message=FALSE, warning=FALSE}

sl501=Fit1@SL50
sl50mk1=Fit1MK@SL50
sl50mk2=Fit2MK@SL50
sl50mk3=Fit3MK@SL50
sl50L1=Fit1Linf@SL50
sl50L2=Fit2Linf@SL50

min1=min(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))
max1=max(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))






sl951=Fit1@SL95
sl95mk1=Fit1MK@SL95
sl95mk2=Fit2MK@SL95
sl95mk3=Fit3MK@SL95
sl95L1=Fit1Linf@SL95
sl95L2=Fit2Linf@SL95

min2=min(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))
max2=max(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))

min=min(min1,min2)
max=max(max1,max2)


plot(1982:2019,sl501,ylim=c(min,max),xlab="Years",ylab="50% selectivity estimates",pch=19,main="N. norvegicus FU25 Females")


lines(1982:2019, smoothEsts1MK$SL50, lwd = 1,col=2)
lines(1982:2019, smoothEsts2MK$SL50, lwd = 1,col=3)
lines(1982:2019, smoothEsts3MK$SL50, lwd = 1,col=4)
lines(1982:2019, smoothEsts1Linf$SL50, lwd = 1,col=5)
lines(1982:2019, smoothEsts2Linf$SL50, lwd = 1,col=6)
lines(1982:2019, smoothEsts1$SL50, lwd = 1,col="black")

points(1982:2019, sl50mk1,col=2,pch=19)
points(1982:2019, sl50mk2,col=3,pch=19)
points(1982:2019, sl50mk3,col=4,pch=19)
points(1982:2019, sl50L1,col=5,pch=19)
points(1982:2019, sl50L2,col=6,pch=19)
points(1982:2019, sl501,col="black",pch=19)

legend("topleft", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
plot(1982:2019,sl951,ylim=c(min,max),xlab="Years",ylab="95% selectivity estimates",pch=19,main="N. norvegicus FU25 Females")


lines(1982:2019, smoothEsts1MK$SL95, lwd = 1,col=2)
lines(1982:2019, smoothEsts2MK$SL95, lwd = 1,col=3)
lines(1982:2019, smoothEsts3MK$SL95, lwd = 1,col=4)
lines(1982:2019, smoothEsts1Linf$SL95, lwd = 1,col=5)
lines(1982:2019, smoothEsts2Linf$SL95, lwd = 1,col=6)
lines(1982:2019, smoothEsts1$SL95, lwd = 1,col="black")


points(1982:2019, sl95mk1,col=2,pch=19)
points(1982:2019, sl95mk2,col=3,pch=19)
points(1982:2019, sl95mk3,col=4,pch=19)
points(1982:2019, sl95L1,col=5,pch=19)
points(1982:2019, sl95L2,col=6,pch=19)
points(1982:2019, sl951,col="black",pch=19)
legend("topleft", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```



Underestimated $L_{c}^{LIT}$.


```{r message=FALSE, warning=FALSE}
Hke1ParsL50 <-Hke1Pars
Hke1ParsL50@L50 <- 0.75*Hke1ParsL50@L50
Hke1ParsL50@L95 <- 0.75*Hke1ParsL50@L95

HkeLenFreq1L50 <- new("LB_lengths", LB_pars=Hke1ParsL50, file="FU25_Females.csv", dataType="freq", header=TRUE)
HkeLenFreq1L50@L_units <- Hke1ParsL50@L_units


Fit1L50 <- LBSPRfit(Hke1ParsL50, HkeLenFreq1L50, verbose = FALSE)
```

Overestimated $L_{c}^{LIT}$.


```{r message=FALSE, warning=FALSE}
Hke2ParsL50 <-Hke1Pars
Hke2ParsL50@L50 <- 1.25*Hke2ParsL50@L50
Hke2ParsL50@L95 <- 1.25*Hke2ParsL50@L95

HkeLenFreq2L50 <- new("LB_lengths", LB_pars=Hke2ParsL50, file="FU25_Females.csv", dataType="freq", header=TRUE)
HkeLenFreq2L50@L_units <- Hke2ParsL50@L_units



Fit2L50 <- LBSPRfit(Hke2ParsL50, HkeLenFreq2L50, verbose = FALSE)

```

Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)
smoothEsts1L50 <- data.frame(Fit1L50@Ests)
smoothEsts2L50 <- data.frame(Fit2L50@Ests)


spr1=Fit1@SPR
sprL1=Fit1L50@SPR
sprL2=Fit2L50@SPR



min=min(c(spr1,sprL1,sprL2))
max=max(c(spr1,sprL1,sprL2))

plot(1982:2019,spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(1982:2019, smoothEsts1$SPR, lwd = 1,col="black")
lines(1982:2019, smoothEsts1L50$SPR, lwd = 1,col=5)
lines(1982:2019, smoothEsts2L50$SPR, lwd = 1,col=6)

points(1982:2019, sprL1,col=5,pch=19)
points(1982:2019, sprL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*Lc","1.25*Lc"),
       col=c("black", 5:6), pch=19, cex=0.8)
```




# Nephrops norvegicus FU2627 Females

Life history parameters:
  
$L_{inf}^{LIT}$ =65 mm 
${M/k}^{LIT}$ =2.5
$L_{50}$=26 mm
$L_{95}$=29.9 mm  



```{r message=FALSE, warning=FALSE}
Nepfemales1Pars <- new("LB_pars")
Nepfemales1Pars@Linf <- 65
Nepfemales1Pars@L50 <- 26 
Nepfemales1Pars@L95 <- 29.9
Nepfemales1Pars@MK <- 2.5 
Nepfemales1Pars@L_units <- "mm"
```

Reference model:

```{r message=FALSE, warning=FALSE}
NepfemalesLenFreq1 <- new("LB_lengths", LB_pars=Nepfemales1Pars, file="FU2627_tallasFemales_1988_2019.csv", dataType="freq", header=TRUE)
NepfemalesLenFreq1@L_units <- Nepfemales1Pars@L_units
```



```{r message=FALSE, warning=FALSE}
NepfemalesLenFreq1
Fit1 <- LBSPRfit(Nepfemales1Pars, NepfemalesLenFreq1,verbose = FALSE)
```


Underestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Nepfemales1ParsMK <-Nepfemales1Pars
Nepfemales1ParsMK@MK <- 0.75*2.5


NepfemalesLenFreq1MK <- new("LB_lengths", LB_pars=Nepfemales1ParsMK, file="FU2627_tallasFemales_1988_2019.csv", dataType="freq", header=TRUE)
NepfemalesLenFreq1MK@L_units <- Nepfemales1ParsMK@L_units


Fit1MK <- LBSPRfit(Nepfemales1ParsMK, NepfemalesLenFreq1MK, verbose = FALSE)
```

Overestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Nepfemales2ParsMK <-Nepfemales1Pars
Nepfemales2ParsMK@MK <- 1.25*2.5


NepfemalesLenFreq2MK <- new("LB_lengths", LB_pars=Nepfemales2ParsMK, file="FU2627_tallasFemales_1988_2019.csv", dataType="freq", header=TRUE)
NepfemalesLenFreq2MK@L_units <- Nepfemales2ParsMK@L_units


Fit2MK <- LBSPRfit(Nepfemales2ParsMK, NepfemalesLenFreq2MK,verbose = FALSE)
```

M/k=1.5.

```{r message=FALSE, warning=FALSE}
Nepfemales3ParsMK <-Nepfemales1Pars
Nepfemales3ParsMK@MK <- 1.5

NepfemalesLenFreq3MK <- new("LB_lengths", LB_pars=Nepfemales3ParsMK, file="FU2627_tallasFemales_1988_2019.csv", dataType="freq", header=TRUE)
NepfemalesLenFreq3MK@L_units <- Nepfemales3ParsMK@L_units


Fit3MK <- LBSPRfit(Nepfemales3ParsMK, NepfemalesLenFreq3MK,verbose = FALSE)
```


Underestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Nepfemales1ParsLinf <-Nepfemales1Pars
Nepfemales1ParsLinf@Linf <- 0.75*65 

NepfemalesLenFreq1Linf <- new("LB_lengths", LB_pars=Nepfemales1ParsLinf, file="FU2627_tallasFemales_1988_2019.csv", dataType="freq", header=TRUE)
NepfemalesLenFreq1Linf@L_units <- Nepfemales1ParsLinf@L_units


Fit1Linf <- LBSPRfit(Nepfemales1ParsLinf, NepfemalesLenFreq1Linf,verbose = FALSE)
```

Overestimated $L_{inf}^{LIT}$.


```{r message=FALSE, warning=FALSE}
Nepfemales2ParsLinf <-Nepfemales1Pars
Nepfemales2ParsLinf@Linf <- 1.25*65

NepfemalesLenFreq2Linf <- new("LB_lengths", LB_pars=Nepfemales2ParsLinf, file="FU2627_tallasFemales_1988_2019.csv", dataType="freq", header=TRUE)
NepfemalesLenFreq2Linf@L_units <- Nepfemales2ParsLinf@L_units


Fit2Linf <- LBSPRfit(Nepfemales2ParsLinf, NepfemalesLenFreq2Linf,verbose = FALSE)
```


Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)
smoothEsts1MK <- data.frame(Fit1MK@Ests)
smoothEsts2MK <- data.frame(Fit2MK@Ests)
smoothEsts3MK <- data.frame(Fit3MK@Ests)
smoothEsts1Linf <- data.frame(Fit1Linf@Ests)
smoothEsts2Linf <- data.frame(Fit2Linf@Ests)


spr1=Fit1@SPR
sprmk1=Fit1MK@SPR
sprmk2=Fit2MK@SPR
sprmk3=Fit3MK@SPR
sprL1=Fit1Linf@SPR
sprL2=Fit2Linf@SPR

min=min(c(spr1,sprmk1,sprmk2,sprmk3,sprL1,sprL2))
max=max(c(spr1,sprmk1,sprmk2,sprmk3,sprL1,sprL2))

plot(1988:2019,spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(1988:2019, smoothEsts1$SPR, lwd = 1,col="black")
lines(1988:2019, smoothEsts1MK$SPR, lwd = 1,col=2)
lines(1988:2019, smoothEsts2MK$SPR, lwd = 1,col=3)
lines(1988:2019, smoothEsts3MK$SPR, lwd = 1,col=4)
lines(1988:2019, smoothEsts1Linf$SPR, lwd = 1,col=5)
lines(1988:2019, smoothEsts2Linf$SPR, lwd = 1,col=6)


points(1988:2019, sprmk1,col=2,pch=19)
points(1988:2019, sprmk2,col=3,pch=19)
points(1988:2019, sprmk3,col=4,pch=19)
points(1988:2019, sprL1,col=5,pch=19)
points(1988:2019, sprL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```


```{r message=FALSE, warning=FALSE}

fm1=Fit1@FM
fmmk1=Fit1MK@FM
fmmk2=Fit2MK@FM
fmmk3=Fit3MK@FM
fmL1=Fit1Linf@FM
fmL2=Fit2Linf@FM

min=min(c(fm1,fmmk1,fmmk2,fmmk3,fmL1,fmL2))
max=max(c(fm1,fmmk1,fmmk2,fmmk3,fmL1,fmL2))
plot(1988:2019,fm1,ylim=c(min,max),xlab="Years",ylab="F/M estimates",pch=19)

lines(1988:2019, smoothEsts1$FM, lwd = 1,col="black")
lines(1988:2019, smoothEsts1MK$FM, lwd = 1,col=2)
lines(1988:2019, smoothEsts2MK$FM, lwd = 1,col=3)
lines(1988:2019, smoothEsts3MK$FM, lwd = 1,col=4)
lines(1988:2019, smoothEsts1Linf$FM, lwd = 1,col=5)
lines(1988:2019, smoothEsts2Linf$FM, lwd = 1,col=6)


points(1988:2019, fmmk1,col=2,pch=19)
points(1988:2019, fmmk2,col=3,pch=19)
points(1988:2019, fmmk3,col=4,pch=19)
points(1988:2019, fmL1,col=5,pch=19)
points(1988:2019, fmL2,col=6,pch=19)

legend("topleft", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```


```{r message=FALSE, warning=FALSE}
sl501=Fit1@SL50
sl50mk1=Fit1MK@SL50
sl50mk2=Fit2MK@SL50
sl50mk3=Fit3MK@SL50
sl50L1=Fit1Linf@SL50
sl50L2=Fit2Linf@SL50

min1=min(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))
max1=max(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))



sl951=Fit1@SL95
sl95mk1=Fit1MK@SL95
sl95mk2=Fit2MK@SL95
sl95mk3=Fit3MK@SL95
sl95L1=Fit1Linf@SL95
sl95L2=Fit2Linf@SL95

min2=min(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))
max2=max(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))

min=min(min1,min2)
max=max(max1,max2)


plot(1988:2019,sl501,ylim=c(min,max),xlab="Years",ylab="50% selectivity estimates",pch=19,main="N. norvegicus FU2627 Females")


lines(1988:2019, smoothEsts1MK$SL50, lwd = 1,col=2)
lines(1988:2019, smoothEsts2MK$SL50, lwd = 1,col=3)
lines(1988:2019, smoothEsts3MK$SL50, lwd = 1,col=4)
lines(1988:2019, smoothEsts1Linf$SL50, lwd = 1,col=5)
lines(1988:2019, smoothEsts2Linf$SL50, lwd = 1,col=6)
lines(1988:2019, smoothEsts1$SL50, lwd = 1,col="black")

points(1988:2019, sl50mk1,col=2,pch=19)
points(1988:2019, sl50mk2,col=3,pch=19)
points(1988:2019, sl50mk3,col=4,pch=19)
points(1988:2019, sl50L1,col=5,pch=19)
points(1988:2019, sl50L2,col=6,pch=19)
points(1988:2019, sl501,col="black",pch=19)
legend("topleft", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
plot(1988:2019,sl951,ylim=c(min,max),xlab="Years",ylab="95% selectivity estimates",pch=19,main="N. norvegicus FU2627 Females")


lines(1988:2019, smoothEsts1MK$SL95, lwd = 1,col=2)
lines(1988:2019, smoothEsts2MK$SL95, lwd = 1,col=3)
lines(1988:2019, smoothEsts3MK$SL95, lwd = 1,col=4)
lines(1988:2019, smoothEsts1Linf$SL95, lwd = 1,col=5)
lines(1988:2019, smoothEsts2Linf$SL95, lwd = 1,col=6)
lines(1988:2019, smoothEsts1$SL95, lwd = 1,col="black")


points(1988:2019, sl95mk1,col=2,pch=19)
points(1988:2019, sl95mk2,col=3,pch=19)
points(1988:2019, sl95mk3,col=4,pch=19)
points(1988:2019, sl95L1,col=5,pch=19)
points(1988:2019, sl95L2,col=6,pch=19)
points(1988:2019, sl951,col="black",pch=19)
legend("topleft", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```



Underestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Nepfemales1ParsL50 <-Nepfemales1Pars
Nepfemales1ParsL50@L50 <- 0.75*Nepfemales1ParsL50@L50
Nepfemales1ParsL50@L95 <- 0.75*Nepfemales1ParsL50@L95

NepfemalesLenFreq1L50 <- new("LB_lengths", LB_pars=Nepfemales1ParsL50, file="FU2627_tallasFemales_1988_2019.csv", dataType="freq", header=TRUE)
NepfemalesLenFreq1L50@L_units <- Nepfemales1ParsL50@L_units


Fit1L50 <- LBSPRfit(Nepfemales1ParsL50, NepfemalesLenFreq1L50,verbose = FALSE)
```

Overestimated $L_{c}^{LIT}$.


```{r message=FALSE, warning=FALSE}
Nepfemales2ParsL50 <-Nepfemales1Pars
Nepfemales2ParsL50@L50 <- 1.25*Nepfemales2ParsL50@L50
Nepfemales2ParsL50@L95 <- 1.25*Nepfemales2ParsL50@L95

NepfemalesLenFreq2L50 <- new("LB_lengths", LB_pars=Nepfemales2ParsL50, file="FU2627_tallasFemales_1988_2019.csv", dataType="freq", header=TRUE)
NepfemalesLenFreq2L50@L_units <- Nepfemales2ParsL50@L_units


Fit2L50 <- LBSPRfit(Nepfemales2ParsL50, NepfemalesLenFreq2L50,verbose = FALSE)
```


Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)

smoothEsts1L50 <- data.frame(Fit1L50@Ests)
smoothEsts2L50 <- data.frame(Fit2L50@Ests)


spr1=Fit1@SPR

sprL1=Fit1L50@SPR
sprL2=Fit2L50@SPR

min=min(c(spr1,sprL1,sprL2))
max=max(c(spr1,sprL1,sprL2))

plot(1988:2019,spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(1988:2019, smoothEsts1$SPR, lwd = 1,col="black")

lines(1988:2019, smoothEsts1L50$SPR, lwd = 1,col=5)
lines(1988:2019, smoothEsts2L50$SPR, lwd = 1,col=6)



points(1988:2019, sprL1,col=5,pch=19)
points(1988:2019, sprL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*L50","1.25*L50"),
       col=c("black", 5:6), pch=19, cex=0.8)
```



# Scyliorhinus canicula

Life history parameters:
  
$L_{inf}^{LIT}$ = 75 cm
${M/k}^{LIT}$ =  1.5
$L_{50}$=54.2 cm
$L_{95}$= 58 cm 
  

```{r message=FALSE, warning=FALSE}
PinPars <- new("LB_pars")
PinPars@Linf <- 75
PinPars@L50 <- 54.2 
PinPars@L95 <- 58
PinPars@MK <- 1.5 
PinPars@L_units <- "cm"
```

Reference model:

```{r message=FALSE, warning=FALSE}
PinLenFreq1 <- new("LB_lengths", LB_pars=PinPars, file="Pin8c9afreq.csv", dataType="freq", header=TRUE)
PinLenFreq1@L_units <- PinPars@L_units
PinLenFreq1
Fit1 <- LBSPRfit(PinPars, PinLenFreq1,verbose = FALSE)

```

Underestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pin1ParsMK <-PinPars
Pin1ParsMK@MK <- 0.75*1.5
PinLenFreq1MK <- new("LB_lengths", LB_pars=Pin1ParsMK, file="Pin8c9afreq.csv", dataType="freq", header=TRUE)
PinLenFreq1MK@L_units <- Pin1ParsMK@L_units
Fit1MK <- LBSPRfit(Pin1ParsMK, PinLenFreq1MK,verbose = FALSE)

```

Overestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pin2ParsMK <-PinPars
Pin2ParsMK@MK <- 1.25*1.5
PinLenFreq2MK <- new("LB_lengths", LB_pars=Pin2ParsMK, file="Pin8c9afreq.csv", dataType="freq", header=TRUE)
PinLenFreq2MK@L_units <- Pin2ParsMK@L_units
Fit2MK <- LBSPRfit(Pin2ParsMK, PinLenFreq2MK,verbose = FALSE)

```

Underestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pin1ParsLinf <-PinPars
Pin1ParsLinf@Linf <- 0.75*75 
PinLenFreq1Linf <- new("LB_lengths", LB_pars=Pin1ParsLinf, file="Pin8c9afreq.csv", dataType="freq", header=TRUE)
PinLenFreq1Linf@L_units <- Pin1ParsLinf@L_units
Fit1Linf <- LBSPRfit(Pin1ParsLinf, PinLenFreq1Linf,verbose = FALSE)
```

Overestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pin2ParsLinf <-PinPars
Pin2ParsLinf@Linf <- 1.25*75
PinLenFreq2Linf <- new("LB_lengths", LB_pars=Pin2ParsLinf, file="Pin8c9afreq.csv", dataType="freq", header=TRUE)
PinLenFreq2Linf@L_units <- Pin2ParsLinf@L_units



PinLenFreq2Linf@LMids=seq(20.5,94.5,by=1)
A=PinLenFreq2Linf@LData
for(i in 1:20){
A=rbind(A,(PinLenFreq2Linf@LData[55,]))}
A[55:75,1:6]=rep(0,6)

PinLenFreq2Linf@LData=A
Pin2ParsLinf <-PinPars
Pin2ParsLinf@Linf <-1.25*75
Fit2Linf <- LBSPRfit(Pin2ParsLinf, PinLenFreq2Linf,verbose = FALSE)
```

Plots:


```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)
smoothEsts1MK <- data.frame(Fit1MK@Ests)
smoothEsts2MK <- data.frame(Fit2MK@Ests)
smoothEsts1Linf <- data.frame(Fit1Linf@Ests)
smoothEsts2Linf <- data.frame(Fit2Linf@Ests)

spr1=Fit1@SPR
sprmk1=Fit1MK@SPR
sprmk2=Fit2MK@SPR
sprL1=Fit1Linf@SPR
sprL2=Fit2Linf@SPR

min=min(c(spr1,sprmk1,sprmk2,sprL1,sprL2))
max=max(c(spr1,sprmk1,sprmk2,sprL1,sprL2))

plot(2014:2019,spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(2014:2019, smoothEsts1$SPR, lwd = 1,col="black")
lines(2014:2019, smoothEsts1MK$SPR, lwd = 1,col=2)
lines(2014:2019, smoothEsts2MK$SPR, lwd = 1,col=3)
lines(2014:2019, smoothEsts1Linf$SPR, lwd = 1,col=5)
lines(2014:2019, smoothEsts2Linf$SPR, lwd = 1,col=6)

points(2014:2019, sprmk1,col=2,pch=19)
points(2014:2019, sprmk2,col=3,pch=19)
points(2014:2019, sprL1,col=5,pch=19)
points(2014:2019, sprL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","0.75*Linf","1.25*Linf"),col=c("black", c(2,3,5,6)), pch=19, cex=0.8)

fm1=Fit1@FM
fmmk1=Fit1MK@FM
fmmk2=Fit2MK@FM
fmL1=Fit1Linf@FM
fmL2=Fit2Linf@FM

min=min(c(fm1,fmmk1,fmmk2,fmL1,fmL2))
max=max(c(fm1,fmmk1,fmmk2,fmL1,fmL2))

plot(2014:2019,fm1,ylim=c(min,max),xlab="Years",ylab="F/M estimates",pch=19)

lines(2014:2019, smoothEsts1$FM, lwd = 1,col="black")
lines(2014:2019, smoothEsts1MK$FM, lwd = 1,col=2)
lines(2014:2019, smoothEsts2MK$FM, lwd = 1,col=3)
lines(2014:2019, smoothEsts1Linf$FM, lwd = 1,col=5)
lines(2014:2019, smoothEsts2Linf$FM, lwd = 1,col=6)
points(2014:2019, fmmk1,col=2,pch=19)
points(2014:2019, fmmk2,col=3,pch=19)
points(2014:2019, fmL1,col=5,pch=19)
points(2014:2019, fmL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","0.75*Linf","1.25*Linf"),col=c("black", 2,3,5,6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
sl501=Fit1@SL50
sl50mk1=Fit1MK@SL50
sl50mk2=Fit2MK@SL50
sl50L1=Fit1Linf@SL50
sl50L2=Fit2Linf@SL50

min1=min(c(sl501,sl50mk1,sl50mk2,sl50L1,sl50L2))
max1=max(c(sl501,sl50mk1,sl50mk2,sl50L1,sl50L2))






sl951=Fit1@SL95
sl95mk1=Fit1MK@SL95
sl95mk2=Fit2MK@SL95
sl95L1=Fit1Linf@SL95
sl95L2=Fit2Linf@SL95

min2=min(c(sl951,sl95mk1,sl95mk2,sl95L1,sl95L2))
max2=max(c(sl951,sl95mk1,sl95mk2,sl95L1,sl95L2))

min=min(min1,min2)
max=max(max1,max2)


plot(2014:2019,sl501,ylim=c(min,max),xlab="Years",ylab="50% selectivity estimates",pch=19,main="S. canicula")


lines(2014:2019, smoothEsts1MK$SL50, lwd = 1,col=2)
lines(2014:2019, smoothEsts2MK$SL50, lwd = 1,col=3)
lines(2014:2019, smoothEsts1Linf$SL50, lwd = 1,col=5)
lines(2014:2019, smoothEsts2Linf$SL50, lwd = 1,col=6)
lines(2014:2019, smoothEsts1$SL50, lwd = 1,col="black")

points(2014:2019, sl50mk1,col=2,pch=19)
points(2014:2019, sl50mk2,col=3,pch=19)
points(2014:2019, sl50L1,col=5,pch=19)
points(2014:2019, sl50L2,col=6,pch=19)
points(2014:2019, sl501,col="black",pch=19)
legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","0.75*Linf","1.25*Linf"),col=c("black", 2,3,5,6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
plot(2014:2019,sl951,ylim=c(min,max),xlab="Years",ylab="95% selectivity estimates",pch=19,main="S. canicula")


lines(2014:2019, smoothEsts1MK$SL95, lwd = 1,col=2)
lines(2014:2019, smoothEsts2MK$SL95, lwd = 1,col=3)
lines(2014:2019, smoothEsts1Linf$SL95, lwd = 1,col=5)
lines(2014:2019, smoothEsts2Linf$SL95, lwd = 1,col=6)
lines(2014:2019, smoothEsts1$SL95, lwd = 1,col="black")


points(2014:2019, sl95mk1,col=2,pch=19)
points(2014:2019, sl95mk2,col=3,pch=19)
points(2014:2019, sl95L1,col=5,pch=19)
points(2014:2019, sl95L2,col=6,pch=19)
points(2014:2019, sl951,col="black",pch=19)
legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","0.75*Linf","1.25*Linf"),col=c("black", 2,3,5,6), pch=19, cex=0.8)
```


Underestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pin1ParsL50 <-PinPars
Pin1ParsL50@L50 <- 0.75*Pin1ParsL50@L50
Pin1ParsL50@L95 <- 0.75*Pin1ParsL50@L95

PinLenFreq1L50 <- new("LB_lengths", LB_pars=Pin1ParsL50, file="Pin8c9afreq.csv", dataType="freq", header=TRUE)
PinLenFreq1L50@L_units <- Pin1ParsL50@L_units
Fit1L50 <- LBSPRfit(Pin1ParsL50, PinLenFreq1L50,verbose = FALSE)
```

Overestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pin2ParsL50 <-PinPars
Pin2ParsL50@L50 <- 1.25*Pin2ParsL50@L50
Pin2ParsL50@L95 <- 1.25*Pin2ParsL50@L95

PinLenFreq2L50 <- new("LB_lengths", LB_pars=Pin2ParsL50, file="Pin8c9afreq.csv", dataType="freq", header=TRUE)
PinLenFreq2L50@L_units <- Pin2ParsL50@L_units




Fit2L50 <- LBSPRfit(Pin2ParsL50, PinLenFreq2L50,verbose = FALSE)
```

Plots:


```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)

smoothEsts1L50 <- data.frame(Fit1L50@Ests)
smoothEsts2L50 <- data.frame(Fit2L50@Ests)

spr1=Fit1@SPR

sprL1=Fit1L50@SPR
sprL2=Fit2L50@SPR



min=min(c(spr1,sprL1,sprL2))
max=max(c(spr1,sprL1,sprL2))

plot(2014:2019,spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(2014:2019, smoothEsts1$SPR, lwd = 1,col="black")

lines(2014:2019, smoothEsts1L50$SPR, lwd = 1,col=5)
lines(2014:2019, smoothEsts2L50$SPR, lwd = 1,col=6)


points(2014:2019, sprL1,col=5,pch=19)
points(2014:2019, sprL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*L50","1.25*L50"),col=c("black", c(5,6)), pch=19, cex=0.8)

```


# Engraulis encrasicolus

Life history parameters:
  
$L_{inf}^{LIT}$ = 18.95 cm
${M/k}^{LIT}$ =  1.44
$L_{50}$=11.2 cm
$L_{95}$=13.3 cm 


```{r message=FALSE, warning=FALSE}
MK<-1.44
LINF<-18.95
Hke1Pars <- new("LB_pars")
Hke1Pars@Linf <- 18.95
Hke1Pars@L50 <- 11.2 
Hke1Pars@L95 <- 13.3
Hke1Pars@MK <- 1.44 
Hke1Pars@L_units <- "cm"
```



```{r message=FALSE, warning=FALSE}
HkeLenFreq1 <- new("LB_lengths", LB_pars=Hke1Pars, file="lengthfreq_anchovy_fv.csv", dataType="freq", header=TRUE)
HkeLenFreq1@L_units <- Hke1Pars@L_units
HkeLenFreq1
Fit1 <- LBSPRfit(Hke1Pars, HkeLenFreq1,verbose =FALSE)

```

Underestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Hke1ParsMK <-Hke1Pars
Hke1ParsMK@MK <- 0.75*MK


HkeLenFreq1MK <- new("LB_lengths", LB_pars=Hke1ParsMK, file="lengthfreq_anchovy_fv.csv", dataType="freq", header=TRUE)
HkeLenFreq1MK@L_units <- Hke1ParsMK@L_units


Fit1MK <- LBSPRfit(Hke1ParsMK, HkeLenFreq1MK,verbose = FALSE)
```

Overestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}


Hke2ParsMK <-Hke1Pars
Hke2ParsMK@MK <- 1.25*MK


HkeLenFreq2MK <- new("LB_lengths", LB_pars=Hke2ParsMK, file="lengthfreq_anchovy_fv.csv", dataType="freq", header=TRUE)
HkeLenFreq2MK@L_units <- Hke2ParsMK@L_units


Fit2MK <- LBSPRfit(Hke2ParsMK, HkeLenFreq2MK,verbose = FALSE)
```

M/k=1.5.

```{r message=FALSE, warning=FALSE}

Hke3ParsMK <-Hke1Pars
Hke3ParsMK@MK <- 1.5

HkeLenFreq3MK <- new("LB_lengths", LB_pars=Hke3ParsMK, file="lengthfreq_anchovy_fv.csv", dataType="freq", header=TRUE)
HkeLenFreq3MK@L_units <- Hke3ParsMK@L_units


Fit3MK <- LBSPRfit(Hke3ParsMK, HkeLenFreq3MK,verbose = FALSE)
```

Underestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}


Hke1ParsLinf <-Hke1Pars
Hke1ParsLinf@Linf <- 0.75*LINF

HkeLenFreq1Linf <- new("LB_lengths", LB_pars=Hke1ParsLinf, file="lengthfreq_anchovy_fv.csv", dataType="freq", header=TRUE)
HkeLenFreq1Linf@L_units <- Hke1ParsLinf@L_units


Fit1Linf <- LBSPRfit(Hke1ParsLinf, HkeLenFreq1Linf,verbose = FALSE)
```

Overestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Hke2ParsLinf <-Hke1Pars
Hke2ParsLinf@Linf <- 1.25*LINF

HkeLenFreq2Linf <- new("LB_lengths", LB_pars=Hke2ParsLinf, file="lengthfreq_anchovy_fv.csv", dataType="freq", header=TRUE)
HkeLenFreq2Linf@L_units <- Hke2ParsLinf@L_units
HkeLenFreq2Linf@LMids=seq(3.75,23.75,0.5)
rows2add<-length(HkeLenFreq2Linf@LMids)-dim(HkeLenFreq2Linf@LData)[1]
B<-array(0,dim=c(rows2add,dim(HkeLenFreq2Linf@LData)[2]))
A=rbind(HkeLenFreq2Linf@LData,B)

HkeLenFreq2Linf@LData=A

Fit2Linf <- LBSPRfit(Hke2ParsLinf, HkeLenFreq2Linf,verbose = FALSE)

```

Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)
smoothEsts1MK <- data.frame(Fit1MK@Ests)
smoothEsts2MK <- data.frame(Fit2MK@Ests)
smoothEsts3MK <- data.frame(Fit3MK@Ests)
smoothEsts1Linf <- data.frame(Fit1Linf@Ests)
smoothEsts2Linf <- data.frame(Fit2Linf@Ests)


spr1=Fit1@SPR
sprmk1=Fit1MK@SPR
sprmk2=Fit2MK@SPR
sprmk3=Fit3MK@SPR
sprL1=Fit1Linf@SPR
sprL2=Fit2Linf@SPR

min=min(c(spr1,sprmk1,sprmk2,sprL1,sprL2))
max=max(c(spr1,sprmk1,sprmk2,sprL1,sprL2))

plot(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32],spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1$SPR, lwd = 1,col="black")
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1MK$SPR, lwd = 1,col=2)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts2MK$SPR, lwd = 1,col=3)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts3MK$SPR, lwd = 1,col=4)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1Linf$SPR, lwd = 1,col=5)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts2Linf$SPR, lwd = 1,col=6)


points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sprmk1,col=2,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sprmk2,col=3,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sprmk3,col=4,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sprL1,col=5,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sprL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)


fm1=Fit1@FM
fmmk1=Fit1MK@FM
fmmk2=Fit2MK@FM
fmmk3=Fit3MK@FM
fmL1=Fit1Linf@FM
fmL2=Fit2Linf@FM

min=min(c(fm1,fmmk1,fmmk2,fmL1,fmL2))
max=max(c(fm1,fmmk1,fmmk2,fmL1,fmL2))

plot(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32],fm1,ylim=c(min,max),xlab="Years",ylab="F/M estimates",pch=19)

lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1$FM, lwd = 1,col="black")
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1MK$FM, lwd = 1,col=2)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts2MK$FM, lwd = 1,col=3)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts3MK$FM, lwd = 1,col=4)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1Linf$FM, lwd = 1,col=5)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts2Linf$FM, lwd = 1,col=6)


points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], fmmk1,col=2,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], fmmk2,col=3,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], fmmk3,col=4,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], fmL1,col=5,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], fmL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
sl501=Fit1@SL50
sl50mk1=Fit1MK@SL50
sl50mk2=Fit2MK@SL50
sl50mk3=Fit3MK@SL50
sl50L1=Fit1Linf@SL50
sl50L2=Fit2Linf@SL50

min1=min(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))
max1=max(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))






sl951=Fit1@SL95
sl95mk1=Fit1MK@SL95
sl95mk2=Fit2MK@SL95
sl95mk3=Fit3MK@SL95
sl95L1=Fit1Linf@SL95
sl95L2=Fit2Linf@SL95

min2=min(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))
max2=max(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))

min=min(min1,min2)
max=max(max1,max2)


plot(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32],sl501,ylim=c(min,max),xlab="Years",ylab="50% selectivity estimates",pch=19,main="E. encrasicolus")


lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1MK$SL50, lwd = 1,col=2)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts2MK$SL50, lwd = 1,col=3)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts3MK$SL50, lwd = 1,col=4)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1Linf$SL50, lwd = 1,col=5)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts2Linf$SL50, lwd = 1,col=6)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1$SL50, lwd = 1,col="black")

points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl50mk1,col=2,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl50mk2,col=3,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl50mk3,col=4,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl50L1,col=5,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl50L2,col=6,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl501,col="black",pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
plot(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32],sl951,ylim=c(min,max),xlab="Years",ylab="95% selectivity estimates",pch=19,main="E. encrasicolus")


lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1MK$SL95, lwd = 1,col=2)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts2MK$SL95, lwd = 1,col=3)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts3MK$SL95, lwd = 1,col=4)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1Linf$SL95, lwd = 1,col=5)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts2Linf$SL95, lwd = 1,col=6)
lines(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], smoothEsts1$SL95, lwd = 1,col="black")


points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl95mk1,col=2,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl95mk2,col=3,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl95mk3,col=4,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl95L1,col=5,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl95L2,col=6,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sl951,col="black",pch=19)

legend("topright", legend=c("REFERENCIA", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```



Underestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}


Hke1ParsL50 <-Hke1Pars
Hke1ParsL50@L50 <- 0.75*Hke1ParsL50@L50
Hke1ParsL50@L95 <- 0.75*Hke1ParsL50@L95


HkeLenFreq1L50 <- new("LB_lengths", LB_pars=Hke1ParsL50, file="lengthfreq_anchovy_fv.csv", dataType="freq", header=TRUE)
HkeLenFreq1L50@L_units <- Hke1ParsL50@L_units


Fit1L50 <- LBSPRfit(Hke1ParsL50, HkeLenFreq1L50,verbose = FALSE)
```

Overestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Hke2ParsL50 <-Hke1Pars
Hke2ParsL50@L50 <- 1.25*Hke2ParsL50@L50
Hke2ParsL50@L95 <- 1.25*Hke2ParsL50@L95


HkeLenFreq2L50 <- new("LB_lengths", LB_pars=Hke2ParsL50, file="lengthfreq_anchovy_fv.csv", dataType="freq", header=TRUE)
HkeLenFreq2L50@L_units <- Hke2ParsL50@L_units


Fit2L50 <- LBSPRfit(Hke2ParsL50, HkeLenFreq2L50,verbose = FALSE)

```

Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)

smoothEsts1L50 <- data.frame(Fit1L50@Ests)
smoothEsts2L50 <- data.frame(Fit2L50@Ests)


spr1=Fit1@SPR

sprL1=Fit1L50@SPR
sprL2=Fit2L50@SPR

min=min(c(spr1,sprL1,sprL2))
max=max(c(spr1,sprL1,sprL2))

plot(HkeLenFreq2L50@Years[1]:HkeLenFreq2L50@Years[32],spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(HkeLenFreq2L50@Years[1]:HkeLenFreq2L50@Years[32], smoothEsts1$SPR, lwd = 1,col="black")

lines(HkeLenFreq2L50@Years[1]:HkeLenFreq2L50@Years[32], smoothEsts1L50$SPR, lwd = 1,col=5)
lines(HkeLenFreq2L50@Years[1]:HkeLenFreq2L50@Years[32], smoothEsts2L50$SPR, lwd = 1,col=6)



points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sprL1,col=5,pch=19)
points(HkeLenFreq2Linf@Years[1]:HkeLenFreq2Linf@Years[32], sprL2,col=6,pch=19)

legend("topright", legend=c("REFERENCIA","0.75*Lc","1.25*Lc"),
       col=c("black", 5:6), pch=19, cex=0.8)

```

# Pollachius pollachius

Life history parameters:
  
$L_{inf}^{LIT}$ = 98.2 cm
${M/k}^{LIT}$ =1.648352
$L_{50}$= 42.3 cm
$L_{95}$=  59 cm



```{r message=FALSE, warning=FALSE}

Pol1 <- new("LB_pars")
Pol1@Linf <- 98.2 
Pol1@L50 <- 42.3
Pol1@L95 <- 59 
Pol1@MK <- 0.3/0.182
Pol1@L_units <- "cm"
```

Reference model:

```{r message=FALSE, warning=FALSE}
Len1 <- new("LB_lengths", LB_pars=Pol1, file=("pol89aSPR_MultYr4cm.csv"), dataType="freq",header=TRUE)
Len1
Fit1 <- LBSPRfit(Pol1, Len1,verbose = FALSE)
```

Underestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pol0.75MK <- Pol1
Pol0.75MK@MK <- 0.75*Pol1@MK
Pol0.75LenMK <- new("LB_lengths",  LB_pars=Pol0.75MK, file="pol89aSPR_MultYr4cm.csv", dataType="freq", header=TRUE)
Pol0.75LenMK@L_units <- Pol0.75MK@L_units
Fit0.75MK <- LBSPRfit(Pol0.75MK, Pol0.75LenMK,verbose = FALSE)
```

Overestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pol1.25MK <- Pol1
Pol1.25MK@MK <- 1.25*Pol1@MK
Pol1.25LenMK <- new("LB_lengths", LB_pars=Pol1.25MK, file="pol89aSPR_MultYr4cm.csv", dataType="freq", header=TRUE)
Pol1.25LenMK@L_units <- Pol1.25MK@L_units
Fit1.25MK <- LBSPRfit(Pol1.25MK, Pol1.25LenMK,verbose = FALSE)
```

M/k=1.5.

```{r message=FALSE, warning=FALSE}
Pol1.5MK <- Pol1
Pol1.5MK@MK <- 1.5
Pol1.5LenMK <- new("LB_lengths", LB_pars=Pol1.5MK, file="pol89aSPR_MultYr4cm.csv", dataType="freq", header=TRUE)
Pol1.5LenMK@L_units <- Pol1.5MK@L_units
Fit1.5MK <- LBSPRfit(Pol1.5MK, Pol1.5LenMK,verbose = FALSE)
```

Underestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pol0.75Linf <- Pol1
Pol0.75Linf@Linf <- 0.75*Pol1@Linf
Pol0.75LenLinf <- new("LB_lengths", LB_pars=Pol0.75Linf, file="pol89aSPR_MultYr4cm.csv", dataType="freq", header=TRUE)
Pol0.75LenLinf@L_units <- Pol0.75Linf@L_units
Fit0.75Linf <- LBSPRfit(Pol0.75Linf, Pol0.75LenLinf,verbose = FALSE)
```

Overestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pol1.25Linf <- Pol1
Pol1.25Linf@Linf <- 1.25*Pol1@Linf
Pol1.25LenLinf <- new("LB_lengths", LB_pars=Pol1.25Linf, file="pol89aSPR_MultYr4cm.csv", dataType="freq", header=TRUE)
Pol1.25LenLinf@L_units <- Pol1.25Linf@L_units
Fit1.25Linf <- LBSPRfit(Pol1.25Linf, Pol1.25LenLinf,verbose = FALSE)
```

Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)
smoothEsts1MK <- data.frame(Fit0.75MK@Ests)
smoothEsts2MK <- data.frame(Fit1.25MK@Ests)
smoothEsts3MK <- data.frame(Fit1.5MK@Ests)
smoothEsts1Linf <- data.frame(Fit0.75Linf@Ests)
smoothEsts2Linf <- data.frame(Fit1.25Linf@Ests)

spr1 <- Fit1@SPR
sprmk1 <- Fit0.75MK@SPR
sprmk2 <- Fit1.25MK@SPR
sprmk3 <- Fit1.5MK@SPR
sprL1 <- Fit0.75Linf@SPR
sprL2 <- Fit1.25Linf@SPR

min=min(c(spr1,sprmk1,sprmk2,sprL1,sprL2))
max=max(c(spr1,sprmk1,sprmk2,sprL1,sprL2))

plot(c(2010:2019),spr1,ylim=c(min,max),xlab="Year",ylab="SPR estimate",pch=19)

lines(2010:2019, smoothEsts1$SPR, lwd = 1,col="black")
lines(2010:2019, smoothEsts1MK$SPR, lwd = 1, col=2)
lines(2010:2019, smoothEsts2MK$SPR, lwd = 1, col=3)
lines(2010:2019, smoothEsts3MK$SPR, lwd = 1, col=4)
lines(2010:2019, smoothEsts1Linf$SPR, lwd = 1, col=5)
lines(2010:2019, smoothEsts2Linf$SPR, lwd = 1 ,col=6)


points(2010:2019, sprmk1,col=2,pch=19)
points(2010:2019, sprmk2,col=3,pch=19)
points(2010:2019, sprmk3,col=4,pch=19)
points(2010:2019, sprL1,col=5,pch=19)
points(2010:2019, sprL2,col=6, pch=19)

legend("topright", legend=c("Reference Case", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)


fm1=Fit1@FM
fmmk1=Fit0.75MK@FM
fmmk2=Fit1.25MK@FM
fmmk3=Fit1.5MK@FM
fmL1=Fit0.75Linf@FM
fmL2=Fit1.25Linf@FM

min=min(c(fm1,fmmk1,fmmk2,fmL1,fmL2))
max=max(c(fm1,fmmk1,fmmk2,fmL1,fmL2))

plot(2010:2019,fm1,ylim=c(min,max),xlab="Year",ylab="F/M estimate",pch=19)

lines(2010:2019, smoothEsts1$FM, lwd = 1,col="black")
lines(2010:2019, smoothEsts1MK$FM, lwd = 1,col=2)
lines(2010:2019, smoothEsts2MK$FM, lwd = 1,col=3)
lines(2010:2019, smoothEsts3MK$FM, lwd = 1,col=4)
lines(2010:2019, smoothEsts1Linf$FM, lwd = 1, col=5)
lines(2010:2019, smoothEsts2Linf$FM, lwd = 1, col=6)


points(2010:2019, fmmk1,col=2,pch=19)
points(2010:2019, fmmk2,col=3,pch=19)
points(2010:2019, fmmk3,col=4,pch=19)
points(2010:2019, fmL1,col=5,pch=19)
points(2010:2019, fmL2,col=6,pch=19)
legend("topright", legend=c("Reference Case", "0.75*M/k","1.25*M/k","M/k=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```



```{r message=FALSE, warning=FALSE}
sl501=Fit1@SL50
sl50mk1=Fit0.75MK@SL50
sl50mk2=Fit1.25MK@SL50
sl50mk3=Fit1.5MK@SL50
sl50L1=Fit0.75Linf@SL50
sl50L2=Fit1.25Linf@SL50

min1=min(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))
max1=max(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))



sl951=Fit1@SL95
sl95mk1=Fit0.75MK@SL95
sl95mk2=Fit1.25MK@SL95
sl95mk3=Fit1.5MK@SL95
sl95L1=Fit0.75Linf@SL95
sl95L2=Fit1.25Linf@SL95

min2=min(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))
max2=max(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))

min=min(min1,min2)
max=max(max1,max2)


plot(2010:2019,sl501,ylim=c(min,max),xlab="Years",ylab="50% selectivity estimates",pch=19,main="P. pollachius")


lines(2010:2019, smoothEsts1MK$SL50, lwd = 1,col=2)
lines(2010:2019, smoothEsts2MK$SL50, lwd = 1,col=3)
lines(2010:2019, smoothEsts3MK$SL50, lwd = 1,col=4)
lines(2010:2019, smoothEsts1Linf$SL50, lwd = 1,col=5)
lines(2010:2019, smoothEsts2Linf$SL50, lwd = 1,col=6)
lines(2010:2019, smoothEsts1$SL50, lwd = 1,col="black")

points(2010:2019, sl50mk1,col=2,pch=19)
points(2010:2019, sl50mk2,col=3,pch=19)
points(2010:2019, sl50mk3,col=4,pch=19)
points(2010:2019, sl50L1,col=5,pch=19)
points(2010:2019, sl50L2,col=6,pch=19)
points(2010:2019, sl501,col="black",pch=19)
legend("topright", legend=c("Reference Case", "0.75*M/k","1.25*M/k","M/k=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
plot(2010:2019,sl951,ylim=c(min,max),xlab="Years",ylab="95% selectivity estimates",pch=19,main="P. pollachius")


lines(2010:2019, smoothEsts1MK$SL95, lwd = 1,col=2)
lines(2010:2019, smoothEsts2MK$SL95, lwd = 1,col=3)
lines(2010:2019, smoothEsts3MK$SL95, lwd = 1,col=4)
lines(2010:2019, smoothEsts1Linf$SL95, lwd = 1,col=5)
lines(2010:2019, smoothEsts2Linf$SL95, lwd = 1,col=6)
lines(2010:2019, smoothEsts1$SL95, lwd = 1,col="black")


points(2010:2019, sl95mk1,col=2,pch=19)
points(2010:2019, sl95mk2,col=3,pch=19)
points(2010:2019, sl95mk3,col=4,pch=19)
points(2010:2019, sl95L1,col=5,pch=19)
points(2010:2019, sl95L2,col=6,pch=19)
points(2010:2019, sl951,col="black",pch=19)
legend("topright", legend=c("Reference Case", "0.75*M/k","1.25*M/k","M/k=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```


Underestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pol0.75L50 <- Pol1
Pol0.75L50@L50 <- 0.75*Pol1@L50
Pol0.75L50@L95 <- 0.75*Pol1@L95

Pol0.75LenL50 <- new("LB_lengths", LB_pars=Pol0.75L50, file="pol89aSPR_MultYr4cm.csv", dataType="freq", header=TRUE)
Pol0.75LenL50@L_units <- Pol0.75L50@L_units
Fit0.75L50 <- LBSPRfit(Pol0.75L50, Pol0.75LenL50,verbose = FALSE)
```

Overestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
Pol1.25L50 <- Pol1
Pol1.25L50@L50 <- 1.25*Pol1@L50
Pol1.25L50@L95 <- 1.25*Pol1@L95

Pol1.25LenL50 <- new("LB_lengths", LB_pars=Pol1.25L50, file="pol89aSPR_MultYr4cm.csv", dataType="freq", header=TRUE)
Pol1.25LenL50@L_units <- Pol1.25L50@L_units
Fit1.25L50 <- LBSPRfit(Pol1.25L50, Pol1.25LenL50,verbose = FALSE)
```

Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)

smoothEsts1L50 <- data.frame(Fit0.75L50@Ests)
smoothEsts2L50 <- data.frame(Fit1.25L50@Ests)

spr1 <- Fit1@SPR

sprL1 <- Fit0.75L50@SPR
sprL2 <- Fit1.25L50@SPR

min=min(c(spr1,sprL1,sprL2))
max=max(c(spr1,sprL1,sprL2))

plot(c(2010:2019),spr1,ylim=c(min,max),xlab="Year",ylab="SPR estimate",pch=19)

lines(2010:2019, smoothEsts1$SPR, lwd = 1,col="black")

lines(2010:2019, smoothEsts1L50$SPR, lwd = 1, col=5)
lines(2010:2019, smoothEsts2L50$SPR, lwd = 1 ,col=6)



points(2010:2019, sprL1,col=5,pch=19)
points(2010:2019, sprL2,col=6, pch=19)

legend("topright", legend=c("Reference Case","0.75*L50","1.25*L50"),
       col=c("black", 5:6), pch=19, cex=0.8)
```

# Pagellus bogaraveo

Life history parameters:
  
$L_{inf}^{LIT}$ = 62 cm
${M/k}^{LIT}$ = 1.42
$L_{50}$= 33 cm
$L_{95}$= 35 cm



```{r message=FALSE, warning=FALSE}
sbr1Pars <- new("LB_pars")
sbr1Pars@Linf <- 62
sbr1Pars@L50 <- 33 
sbr1Pars@L95 <- 35
sbr1Pars@MK <- 1.42 
sbr1Pars@L_units <- "cm"
```

Reference model:

```{r message=FALSE, warning=FALSE}
sbrLenFreq1 <- new("LB_lengths", LB_pars=sbr1Pars, file="LFreqSBR.csv", dataType="freq", header=TRUE)
sbrLenFreq1@L_units <- sbr1Pars@L_units
sbrLenFreq1
Fit1 <- LBSPRfit(sbr1Pars, sbrLenFreq1,verbose = FALSE)
```

Underestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
sbr1ParsMK <-sbr1Pars
sbr1ParsMK@MK <- 0.75*1.42

sbrLenFreq1MK <- new("LB_lengths", LB_pars=sbr1ParsMK, file="LFreqSBR.csv", dataType="freq", header=TRUE)
sbrLenFreq1MK@L_units <- sbr1ParsMK@L_units

Fit1MK <- LBSPRfit(sbr1ParsMK, sbrLenFreq1MK,verbose = FALSE)
```

Overestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
sbr2ParsMK <-sbr1Pars
sbr2ParsMK@MK <- 1.25*1.42

sbrLenFreq2MK <- new("LB_lengths", LB_pars=sbr2ParsMK, file="LFreqSBR.csv", dataType="freq", header=TRUE)
sbrLenFreq2MK@L_units <- sbr2ParsMK@L_units

Fit2MK <- LBSPRfit(sbr2ParsMK, sbrLenFreq2MK,verbose = FALSE)
```

M/k=1.5.

```{r message=FALSE, warning=FALSE}
sbr3ParsMK <-sbr1Pars
sbr3ParsMK@MK <- 1.5

sbrLenFreq3MK <- new("LB_lengths", LB_pars=sbr3ParsMK, file="LFreqSBR.csv", dataType="freq", header=TRUE)
sbrLenFreq3MK@L_units <- sbr3ParsMK@L_units

Fit3MK <- LBSPRfit(sbr3ParsMK, sbrLenFreq3MK,verbose = FALSE)

```

Underestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
sbr1ParsLinf <-sbr1Pars
sbr1ParsLinf@Linf <- 62*0.75

sbrLenFreq1Linf <- new("LB_lengths", LB_pars=sbr1ParsLinf, file="LFreqSBR.csv", dataType="freq", header=TRUE)
sbrLenFreq1Linf@L_units <- sbr1ParsLinf@L_units

Fit1Linf <- LBSPRfit(sbr1ParsLinf, sbrLenFreq1Linf,verbose = FALSE)

```

Overestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
sbr2ParsLinf <-sbr1Pars
sbr2ParsLinf@Linf <- 62*1.25

sbrLenFreq2Linf <- new("LB_lengths", LB_pars=sbr2ParsLinf, file="LFreqSBR.csv", dataType="freq", header=TRUE)
sbrLenFreq2Linf@L_units <- sbr2ParsLinf@L_units

Fit2Linf <- LBSPRfit(sbr2ParsLinf, sbrLenFreq2Linf,verbose = FALSE)
```

Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)
smoothEsts1MK <- data.frame(Fit1MK@Ests)
smoothEsts2MK <- data.frame(Fit2MK@Ests)
smoothEsts3MK <- data.frame(Fit3MK@Ests)
smoothEsts1Linf <- data.frame(Fit1Linf@Ests)
smoothEsts2Linf <- data.frame(Fit2Linf@Ests)

spr1=Fit1@SPR
sprmk1=Fit1MK@SPR
sprmk2=Fit2MK@SPR
sprmk3=Fit3MK@SPR
sprL1=Fit1Linf@SPR
sprL2=Fit2Linf@SPR

min=min(c(spr1,sprmk1,sprmk2,sprmk3,sprL1,sprL2))
max=max(c(spr1,sprmk1,sprmk2,sprmk3,sprL1,sprL2))

plot(1997:2019,spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(1997:2019, smoothEsts1$SPR, lwd = 1,col="black")
lines(1997:2019, smoothEsts1MK$SPR, lwd = 1,col=2)
lines(1997:2019, smoothEsts2MK$SPR, lwd = 1,col=3)
lines(1997:2019, smoothEsts3MK$SPR, lwd = 1,col=4)
lines(1997:2019, smoothEsts1Linf$SPR, lwd = 1,col=5)
lines(1997:2019, smoothEsts2Linf$SPR, lwd = 1,col=6)

points(1997:2019, sprmk1,col=2,pch=19)
points(1997:2019, sprmk2,col=3,pch=19)
points(1997:2019, sprmk3,col=4,pch=19)
points(1997:2019, sprL1,col=5,pch=19)
points(1997:2019, sprL2,col=6,pch=19)

legend("topright", legend=c("BASE", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)

fm1=Fit1@FM
fmmk1=Fit1MK@FM
fmmk2=Fit2MK@FM
fmmk3=Fit3MK@FM
fmL1=Fit1Linf@FM
fmL2=Fit2Linf@FM

min=min(c(fm1,fmmk1,fmmk2,fmmk3,fmL1,fmL2))
max=max(c(fm1,fmmk1,fmmk2,fmmk3,fmL1,fmL2))

plot(1997:2019,fm1,ylim=c(min,max),xlab="Years",ylab="F/M estimates",pch=19)

lines(1997:2019, smoothEsts1$FM, lwd = 1,col="black")
lines(1997:2019, smoothEsts1MK$FM, lwd = 1,col=2)
lines(1997:2019, smoothEsts2MK$FM, lwd = 1,col=3)
lines(1997:2019, smoothEsts3MK$FM, lwd = 1,col=4)
lines(1997:2019, smoothEsts1Linf$FM, lwd = 1,col=5)
lines(1997:2019, smoothEsts2Linf$FM, lwd = 1,col=6)

points(1997:2019, fmmk1,col=2,pch=19)
points(1997:2019, fmmk2,col=3,pch=19)
points(1997:2019, fmmk3,col=4,pch=19)
points(1997:2019, fmL1,col=5,pch=19)
points(1997:2019, fmL2,col=6,pch=19)

legend("topleft", legend=c("BASE", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
sl501=Fit1@SL50
sl50mk1=Fit1MK@SL50
sl50mk2=Fit2MK@SL50
sl50mk3=Fit3MK@SL50
sl50L1=Fit1Linf@SL50
sl50L2=Fit2Linf@SL50

min1=min(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))
max1=max(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))






sl951=Fit1@SL95
sl95mk1=Fit1MK@SL95
sl95mk2=Fit2MK@SL95
sl95mk3=Fit3MK@SL95
sl95L1=Fit1Linf@SL95
sl95L2=Fit2Linf@SL95

min2=min(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))
max2=max(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))

min=min(min1,min2)
max=max(max1,max2)


plot(1997:2019,sl501,ylim=c(min,max),xlab="Years",ylab="50% selectivity estimates",pch=19,main="P. bogaraveo")


lines(1997:2019, smoothEsts1MK$SL50, lwd = 1,col=2)
lines(1997:2019, smoothEsts2MK$SL50, lwd = 1,col=3)
lines(1997:2019, smoothEsts3MK$SL50, lwd = 1,col=4)
lines(1997:2019, smoothEsts1Linf$SL50, lwd = 1,col=5)
lines(1997:2019, smoothEsts2Linf$SL50, lwd = 1,col=6)
lines(1997:2019, smoothEsts1$SL50, lwd = 1,col="black")

points(1997:2019, sl50mk1,col=2,pch=19)
points(1997:2019, sl50mk2,col=3,pch=19)
points(1997:2019, sl50mk3,col=4,pch=19)
points(1997:2019, sl50L1,col=5,pch=19)
points(1997:2019, sl50L2,col=6,pch=19)
points(1997:2019, sl501,col="black",pch=19)

legend("topleft", legend=c("BASE", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```

```{r message=FALSE, warning=FALSE}
plot(1997:2019,sl951,ylim=c(min,max),xlab="Years",ylab="95% selectivity estimates",pch=19,main="P. bogaraveo")


lines(1997:2019, smoothEsts1MK$SL95, lwd = 1,col=2)
lines(1997:2019, smoothEsts2MK$SL95, lwd = 1,col=3)
lines(1997:2019, smoothEsts3MK$SL95, lwd = 1,col=4)
lines(1997:2019, smoothEsts1Linf$SL95, lwd = 1,col=5)
lines(1997:2019, smoothEsts2Linf$SL95, lwd = 1,col=6)
lines(1997:2019, smoothEsts1$SL95, lwd = 1,col="black")


points(1997:2019, sl95mk1,col=2,pch=19)
points(1997:2019, sl95mk2,col=3,pch=19)
points(1997:2019, sl95mk3,col=4,pch=19)
points(1997:2019, sl95L1,col=5,pch=19)
points(1997:2019, sl95L2,col=6,pch=19)
points(1997:2019, sl951,col="black",pch=19)

legend("topleft", legend=c("BASE", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```


Underestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
sbr1ParsL50 <-sbr1Pars
sbr1ParsL50@L50 <- sbr1ParsL50@L50*0.75
sbr1ParsL50@L95 <- sbr1ParsL50@L95*0.75

sbrLenFreq1L50 <- new("LB_lengths", LB_pars=sbr1ParsL50, file="LFreqSBR.csv", dataType="freq", header=TRUE)
sbrLenFreq1L50@L_units <- sbr1ParsL50@L_units

Fit1L50 <- LBSPRfit(sbr1ParsL50, sbrLenFreq1L50,verbose = FALSE)

```

Overestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
sbr2ParsL50 <-sbr1Pars
sbr2ParsL50@L50 <- sbr2ParsL50@L50*1.25
sbr2ParsL50@L95 <- sbr2ParsL50@L95*1.25

sbrLenFreq2L50 <- new("LB_lengths", LB_pars=sbr2ParsL50, file="LFreqSBR.csv", dataType="freq", header=TRUE)
sbrLenFreq2L50@L_units <- sbr2ParsL50@L_units

Fit2L50 <- LBSPRfit(sbr2ParsL50, sbrLenFreq2L50,verbose = FALSE)
```

Plots:

```{r message=FALSE, warning=FALSE}

smoothEsts1 <- data.frame(Fit1@Ests)

smoothEsts1L50 <- data.frame(Fit1L50@Ests)
smoothEsts2L50 <- data.frame(Fit2L50@Ests)

spr1=Fit1@SPR

sprL1=Fit1L50@SPR
sprL2=Fit2L50@SPR

min=min(c(spr1,sprL1,sprL2))
max=max(c(spr1,sprL1,sprL2))

plot(1997:2019,spr1,ylim=c(min,max),xlab="Years",ylab="SPR estimates",pch=19)

lines(1997:2019, smoothEsts1$SPR, lwd = 1,col="black")

lines(1997:2019, smoothEsts1L50$SPR, lwd = 1,col=5)
lines(1997:2019, smoothEsts2L50$SPR, lwd = 1,col=6)

points(1997:2019, sprL1,col=5,pch=19)
points(1997:2019, sprL2,col=6,pch=19)

legend("topright", legend=c("BASE", "0.75*L50","1.25*L50"),
       col=c("black", 2:6), pch=19, cex=0.8)

```

# Trisopterus luscus

Life history parameters:
  
$L_{inf}^{LIT}$ = 46.7 cm
${M/k}^{LIT}$ =  1.76
$L_{50}$= 19.2 cm
$L_{95}$= 21.43 cm

```{r,warning=FALSE,message=FALSE}
library(tidyverse)
library(lubridate)
library(kableExtra)
library(knitr)
MyPars <- new("LB_pars")
MyPars@Species <- "Trisopterus luscus"

LHTs <- data.frame(t(c("Trisopterus luscus", 19.2, 1, 46.7, 0.21, 1.27, 9)))
colnames(LHTs) <- c("Sp", "L50", "Amat", "Linf", "K", "t0", "Amax")
LHTs$MKlit1 <-3.111105
LHTs$MKlit2 <- 1.761846
LHTs$MK <- 1.5

a <- -25.77
b <- 1.34
L95 <- (log(0.95/0.05)-a)/b

MyPars@Linf <- as.numeric(as.character(LHTs$Linf))
MyPars@L50  <- as.numeric(as.character(LHTs$L50 ))
MyPars@L95  <- L95
MyPars@MK   <- as.numeric(as.character(LHTs$MKlit2)) 
MyPars@L_units <- "cm"
```


```{r include=FALSE,eval=FALSE}
library(readr)
newDat <- read_csv("faneca_ieo.csv")

newDat <- data.frame(do.call(cbind, unstack(newDat)))
colnames(newDat) <- seq(1999, 2018, 1)
dim(newDat)
summary(newDat)

write_csv(newDat, "faneca.csv")
```

Reference model:

```{r,warning=FALSE,message=FALSE}
MyLengths <- new("LB_lengths", LB_pars=MyPars, file="pouting.csv", 
                 dataType="freq", header = TRUE)
MyLengths
myFit <- LBSPRfit(MyPars, MyLengths,verbose = FALSE)

```

Underestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
MyParsMKLow <- MyPars
MyParsMKLow@MK <- 0.75*MyPars@MK
MyLengthsMKLow <- new("LB_lengths", LB_pars=MyParsMKLow, file="faneca.csv", 
                 dataType="raw", header = TRUE)
MyLengthsMKLow@L_units <- MyParsMKLow@L_units
myFitMKLow <- LBSPRfit(MyParsMKLow, MyLengthsMKLow,verbose = FALSE)
```

Overestimated ${M/k}^{LIT}$.

```{r message=FALSE, warning=FALSE}
MyParsMKSup <- MyPars
MyParsMKSup@MK <- 1.25*MyPars@MK
MyLengthsMKSup <- new("LB_lengths", LB_pars=MyParsMKSup, file="faneca.csv", 
                     dataType="raw", header = TRUE)
MyLengthsMKSup@L_units <- MyParsMKSup@L_units
myFitMKSup <- LBSPRfit(MyParsMKSup, MyLengthsMKSup,verbose = FALSE)
```

M/k=1.5.

```{r message=FALSE, warning=FALSE}
MyParsMK15 <- MyPars
MyParsMK15@MK <- 1.5
MyLengthsMK15 <- new("LB_lengths", LB_pars=MyParsMK15, file="faneca.csv", 
                     dataType="raw", header = TRUE)
MyLengthsMK15@L_units <- MyParsMK15@L_units
myFitMK15 <- LBSPRfit(MyParsMK15, MyLengthsMK15,verbose = FALSE)
```

Underestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
MyParsLinfLow <- MyPars
MyParsLinfLow@Linf <- 0.75*MyPars@Linf
MyLengthsLinfLow <- new("LB_lengths", LB_pars=MyParsLinfLow, file="faneca.csv", 
                      dataType="raw", header = TRUE)
MyLengthsLinfLow@L_units <- MyParsLinfLow@L_units
myFitLinfLow <- LBSPRfit(MyParsLinfLow, MyLengthsLinfLow,verbose = FALSE)
```

Overestimated $L_{inf}^{LIT}$.

```{r message=FALSE, warning=FALSE}
MyParsLinfSup <- MyPars
MyParsLinfSup@Linf <- 1.25*MyPars@Linf
MyLengthsLinfSup <- new("LB_lengths", LB_pars=MyParsLinfSup, file="faneca.csv", 
                      dataType="raw",header = TRUE)
MyLengthsLinfSup@L_units <- MyParsLinfSup@L_units
myFitLinfSup <- LBSPRfit(MyParsLinfSup, MyLengthsLinfSup,verbose = FALSE)
```


Plots:

```{r message=FALSE, warning=FALSE}
smoothEsts1        <- data.frame(myFit@Ests)
smoothEsts1MK   <- data.frame(myFitMKLow@Ests)
smoothEsts2MK   <- data.frame(myFitMKSup@Ests)
smoothEsts3MK   <- data.frame(myFitMK15@Ests)
smoothEsts1Linf <- data.frame(myFitLinfLow@Ests)
smoothEsts2Linf <- data.frame(myFitLinfSup@Ests)


spr1     = myFit@SPR
sprmk1   = myFitMKLow@SPR
sprmk2   = myFitMKSup@SPR
sprmk3  = myFitMK15@SPR
sprL1 = myFitLinfLow@SPR
sprL2 = myFitLinfSup@SPR

fm1   = myFit@FM
fmmk1   = myFitMKLow@FM
fmmk2   = myFitMKSup@FM
fmmk3   = myFitMK15@FM
fmL1 = myFitLinfLow@FM
fmL2= myFitLinfSup@FM

min=min(c(spr1,sprmk1,sprmk2,sprmk3,sprL1,sprL2))
max=max(c(spr1,sprmk1,sprmk2,sprmk3,sprL1,sprL2))

plot(1999:2018,spr1,ylim=c(min,max),xlab="Year",ylab="SPR estimate",pch=19)

lines(1999:2018, smoothEsts1$SPR, lwd = 1,col="black")
lines(1999:2018, smoothEsts1MK$SPR, lwd = 1, col=2)
lines(1999:2018, smoothEsts2MK$SPR, lwd = 1, col=3)
lines(1999:2018, smoothEsts3MK$SPR, lwd = 1, col=4)
lines(1999:2018, smoothEsts1Linf$SPR, lwd = 1, col=5)
lines(1999:2018, smoothEsts2Linf$SPR, lwd = 1 ,col=6)


points(1999:2018, sprmk1,col=2,pch=19)
points(1999:2018, sprmk2,col=3,pch=19)
points(1999:2018, sprmk3,col=4,pch=19)
points(1999:2018, sprL1,col=5,pch=19)
points(1999:2018, sprL2,col=6, pch=19)


legend("topright", legend=c("Reference Case", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)

min=min(c(fm1,fmmk1,fmmk2,fmmk3,fmL1,fmL2))
max=max(c(fm1,fmmk1,fmmk2,fmmk3,fmL1,fmL2))

plot(1999:2018,fm1,ylim=c(min,max),xlab="Year",ylab="F/M estimate",pch=19)

lines(1999:2018, smoothEsts1$FM, lwd = 1,col="black")
lines(1999:2018, smoothEsts1MK$FM, lwd = 1,col=2)
lines(1999:2018, smoothEsts2MK$FM, lwd = 1,col=3)
lines(1999:2018, smoothEsts3MK$FM, lwd = 1,col=4)
lines(1999:2018, smoothEsts1Linf$FM, lwd = 1, col=5)
lines(1999:2018, smoothEsts2Linf$FM, lwd = 1, col=6)


points(1999:2018, fmmk1,col=2,pch=19)
points(1999:2018, fmmk2,col=3,pch=19)
points(1999:2018, fmmk3,col=4,pch=19)
points(1999:2018, fmL1,col=5,pch=19)
points(1999:2018, fmL2,col=6,pch=19)

legend("topright", legend=c("Reference Case", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```



```{r message=FALSE, warning=FALSE}
sl501=myFit@SL50
sl50mk1=myFitMKLow@SL50
sl50mk2=myFitMKSup@SL50
sl50mk3=myFitMK15@SL50
sl50L1=myFitLinfLow@SL50
sl50L2=myFitLinfSup@SL50

min1=min(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))
max1=max(c(sl501,sl50mk1,sl50mk2,sl50mk3,sl50L1,sl50L2))






sl951=myFit@SL95
sl95mk1=myFitMKLow@SL95
sl95mk2=myFitMKSup@SL95
sl95mk3=myFitMK15@SL95
sl95L1=myFitLinfLow@SL95
sl95L2=myFitLinfSup@SL95

min2=min(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))
max2=max(c(sl951,sl95mk1,sl95mk2,sl95mk3,sl95L1,sl95L2))

min=min(min1,min2)
max=max(max1,max2)


plot(1999:2018,sl501,ylim=c(min,max),xlab="Years",ylab="50% selectivity estimates",pch=19,main="T. luscus")


lines(1999:2018, smoothEsts1MK$SL50, lwd = 1,col=2)
lines(1999:2018, smoothEsts2MK$SL50, lwd = 1,col=3)
lines(1999:2018, smoothEsts3MK$SL50, lwd = 1,col=4)
lines(1999:2018, smoothEsts1Linf$SL50, lwd = 1,col=5)
lines(1999:2018, smoothEsts2Linf$SL50, lwd = 1,col=6)
lines(1999:2018, smoothEsts1$SL50, lwd = 1,col="black")

points(1999:2018, sl50mk1,col=2,pch=19)
points(1999:2018, sl50mk2,col=3,pch=19)
points(1999:2018, sl50mk3,col=4,pch=19)
points(1999:2018, sl50L1,col=5,pch=19)
points(1999:2018, sl50L2,col=6,pch=19)
points(1999:2018, sl501,col="black",pch=19)

legend("topright", legend=c("Reference Case", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```


```{r message=FALSE, warning=FALSE}
plot(1999:2018,sl951,ylim=c(min,max),xlab="Years",ylab="95% selectivity estimates",pch=19,main="T. luscus")


lines(1999:2018, smoothEsts1MK$SL95, lwd = 1,col=2)
lines(1999:2018, smoothEsts2MK$SL95, lwd = 1,col=3)
lines(1999:2018, smoothEsts3MK$SL95, lwd = 1,col=4)
lines(1999:2018, smoothEsts1Linf$SL95, lwd = 1,col=5)
lines(1999:2018, smoothEsts2Linf$SL95, lwd = 1,col=6)
lines(1999:2018, smoothEsts1$SL95, lwd = 1,col="black")


points(1999:2018, sl95mk1,col=2,pch=19)
points(1999:2018, sl95mk2,col=3,pch=19)
points(1999:2018, sl95mk3,col=4,pch=19)
points(1999:2018, sl95L1,col=5,pch=19)
points(1999:2018, sl95L2,col=6,pch=19)
points(1999:2018, sl951,col="black",pch=19)

legend("topright", legend=c("Reference Case", "0.75*MK","1.25*MK","MK=1.5","0.75*Linf","1.25*Linf"),
       col=c("black", 2:6), pch=19, cex=0.8)
```


Underestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
MyParsL50Low <- MyPars
MyParsL50Low@L50 <- 0.75*MyPars@L50
MyParsL50Low@L95 <- 0.75*MyPars@L95
MyLengthsL50Low <- new("LB_lengths", LB_pars=MyParsL50Low, file="faneca.csv", 
                      dataType="raw", header = TRUE)
MyLengthsL50Low@L_units <- MyParsL50Low@L_units
myFitL50Low <- LBSPRfit(MyParsL50Low, MyLengthsL50Low,verbose = FALSE)
```

Overestimated $L_{c}^{LIT}$.

```{r message=FALSE, warning=FALSE}
MyParsL50Sup <- MyPars
MyParsL50Sup@L50 <- 1.25*MyPars@L50
MyParsL50Sup@L95 <- 1.25*MyPars@L95
MyLengthsL50Sup <- new("LB_lengths", LB_pars=MyParsL50Sup, file="faneca.csv", 
                      dataType="raw",header = TRUE)
MyLengthsL50Sup@L_units <- MyParsL50Sup@L_units
myFitL50Sup <- LBSPRfit(MyParsL50Sup, MyLengthsL50Sup,verbose = FALSE)
```


Plots:

```{r message=FALSE, warning=FALSE}
smoothEsts1        <- data.frame(myFit@Ests)

smoothEsts1L50 <- data.frame(myFitL50Low@Ests)
smoothEsts2L50 <- data.frame(myFitL50Sup@Ests)


spr1     = myFit@SPR

sprL1 = myFitL50Low@SPR
sprL2 = myFitL50Sup@SPR

fm1   = myFit@FM

fmL1 = myFitL50Low@FM
fmL2= myFitL50Sup@FM

min=min(c(spr1,sprL1,sprL2))
max=max(c(spr1,sprL1,sprL2))

plot(1999:2018,spr1,ylim=c(min,max),xlab="Year",ylab="SPR estimate",pch=19)

lines(1999:2018, smoothEsts1$SPR, lwd = 1,col="black")
lines(1999:2018, smoothEsts1L50$SPR, lwd = 1, col=5)
lines(1999:2018, smoothEsts2L50$SPR, lwd = 1 ,col=6)



points(1999:2018, sprL1,col=5,pch=19)
points(1999:2018, sprL2,col=6, pch=19)


legend("topright", legend=c("Reference Case","0.75*L50","1.25*L50"),
       col=c("black", 5:6), pch=19, cex=0.8)
```